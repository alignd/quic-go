...
var _ = Describe("Varint packet number encoding / decoding", func() {
	Context("Decoding", func() {
		...
		It("errors on EOF when reading a 2 byte number", func() {
			...
			_, err := ReadVarIntPacketNumber(b)
			Expect(err).To(MatchError(io.EOF))
		})
	...
	Context("Encoding", func() {
		It("writes a 1 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0x42, protocol.PacketNumberLen1)
			Expect(b.Len()).To(Equal(1))
			p, err := ReadVarIntPacketNumber(bytes.NewReader(b.Bytes()))
			Expect(err).ToNot(HaveOccurred())
		...
		It("only uses the least significant 7 bits when writing a 1 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0x1234ea, protocol.PacketNumberLen1)
			Expect(b.Len()).To(Equal(1))
			p, err := ReadVarIntPacketNumber(bytes.NewReader(b.Bytes()))
			Expect(err).ToNot(HaveOccurred())
		...
		It("writes a small 2 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0x42, protocol.PacketNumberLen2)
		...
		It("writes a 2 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0x1337, protocol.PacketNumberLen2)
			Expect(b.Len()).To(Equal(2))
			p, err := ReadVarIntPacketNumber(bytes.NewReader(b.Bytes()))
			Expect(err).ToNot(HaveOccurred())
		...
		It("only uses the least significant 14 bits when writing a 2 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0x1234ff37, protocol.PacketNumberLen2)
			Expect(b.Len()).To(Equal(2))
			p, err := ReadVarIntPacketNumber(bytes.NewReader(b.Bytes()))
			Expect(err).ToNot(HaveOccurred())
		...
		It("writes a small 4 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0xbeef, protocol.PacketNumberLen4)
		...
		It("writes a 4 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0x12beef42, protocol.PacketNumberLen4)
			Expect(b.Len()).To(Equal(4))
			p, err := ReadVarIntPacketNumber(bytes.NewReader(b.Bytes()))
			Expect(err).ToNot(HaveOccurred())
		...
		It("only uses the least significant 30 bits when writing a 4 byte packet number", func() {
			b := &bytes.Buffer{}
			WriteVarIntPacketNumber(b, 0x1234deadbeef, protocol.PacketNumberLen4)
			Expect(b.Len()).To(Equal(4))
			p, err := ReadVarIntPacketNumber(bytes.NewReader(b.Bytes()))
			Expect(err).ToNot(HaveOccurred())
		...
		It("panics when encountering invalid packet number lengths", func() {
			b := &bytes.Buffer{}
			Expect(func() { WriteVarIntPacketNumber(b, 0x1234deadbeef, 13) }).Should(Panic())
		})
	})
})
